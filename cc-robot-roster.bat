@echo off
REM ============================================================
REM   cc-robot-roster (MV-game / Unity 6 LTS) — MV-452
REM
REM   Captures one 1920x1080 screenshot per Backyard robot kind, at
REM   pitch 64.88 / distance 15.81m, via MaxWorlds.Editor.RobotRosterCapture.
REM   Writes to docs\press\robot-roster\ and
REM   C:\Dev\MaxVsTheWorlds-Images\_screens\robot-roster\.
REM   Exit 0 = every shot captured. Read _robotroster_done.txt for detail.
REM
REM   Pre-reqs:
REM     - %UNITY_PATH% env var pointing at Unity.exe (same as cc-verify.bat)
REM     - Run from the repo root.
REM
REM   Deliberately NOT -quit / -nographics: RobotRosterCapture.CaptureAll
...
set "PROJECT=%CD%"
set "LOG=%PROJECT%\Logs\robotroster-run.log"

if not defined UNITY_PATH (
  echo [cc-robot-roster] UNITY_PATH not set. Point it at Unity.exe and retry.
  exit /b 2
)
if not exist "%UNITY_PATH%" (
  echo [cc-robot-roster] UNITY_PATH does not exist: %UNITY_PATH%
  exit /b 2
)
...
echo.
echo === cc-robot-roster (MV-game) start ===
...
if exist "%LOCKFILE%" (
  ...
  if errorlevel 1 (
    echo === cc-robot-roster ABORTED - another Unity instance has this project open ===
    echo     Nothing was captured. This is NOT a capture failure.
    exit /b 3
  ) else (
    echo [cc-robot-roster] stale UnityLockfile found, no live Unity holds it - continuing.
  )
)

"%UNITY_PATH%" ^
  -batchmode -projectPath "%PROJECT%" ^
  -executeMethod MaxWorlds.Editor.RobotRosterCapture.CaptureAll ^
  -logFile "%LOG%"

set "RESULT=%errorlevel%"

echo.
echo === cc-robot-roster (MV-game) end (exit=%RESULT%) ===
