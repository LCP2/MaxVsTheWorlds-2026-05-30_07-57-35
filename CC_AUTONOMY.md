...
**Subagent discipline (MV-592).** If you delegate research or verification to a subagent, wait for its result and use it — do not perform the same work yourself in the main loop while it runs. Duplicating a subagent's work in the generating context (re-reading the same files, re-verifying the same artefacts) defeats the reason for delegating it in the first place: a verifier's value is that it ran separately from whoever generated the thing being checked.
...
- Guardrail trip → stop; set `needs-triage`; state in a comment exactly what tripped, which file and constant, and what you measured. Do NOT set `needs-lee` - the triage worker adjudicates the block and escalates to Lee only when the decision is genuinely his. Then take the next actionable ticket; NEVER IDLE applies unchanged. Specific trips for this project: any engine version change; adding a Unity package not already in `manifest.json`; turning on AI-art generation; expanding a ticket beyond its tight-slice scope; editing a CI / workflow / signing file (see "CI / workflow / signing files — absolute, no exceptions" above); any diff that ADDS a file under `Assets/_Project/Code/Runtime/Dev/` or `Assets/_Project/Code/Editor/` — stop, set `needs-triage`, and name the exact file and why it's needed (MV-592: nine per-ticket capture-director pairs had accumulated this way before the shared `CaptureDirector`/`CaptureRunner` existed; extending or configuring the shared capture director with a new preset is not a trip).
- Physical-world blocker → stop; `blocked-<reason>` + `needs-triage`. Known examples: `blocked-mac` (iOS device build — carry-over, not a present blocker since Windows standalone is the substitute); `blocked-install` (Lee needs to install something).
- **Authored level data is authority.** If a `MapValidation` rule, an EditMode test or any other engine-side check refuses level data that comes from Lee's design workbook - a cover rect, a replicator position or facing, a robot placement, a lane width, an area's connectivity - the check is what is wrong, not the data. Do NOT edit the level data to satisfy it, do NOT weaken or exempt the check on your own authority, and do NOT set `needs-lee`.

  Instead: create a NEW ticket in the `MV` project describing the rule change, labelled `Methodology` + `needs-cc-web`, at the same priority as the current ticket, status `Ready for Dev`. State the rule, the file and constant, the measured value against the threshold, and why the authored data is correct. Link the current ticket as "is blocked by" the new one. Then leave the current ticket where it is and take the next actionable ticket - the NEVER IDLE contract applies unchanged.

  Two exceptions, where `needs-lee` is still correct: (a) the check is derived from Max's own physical body - his CharacterController radius, a collider clearance - and relaxing it would let the player clip or stick, and (b) the ticket's own description explicitly instructs you to block on that rule by name.
- `needs-lee` is not yours to set. It is set by the triage worker, or by a human. If you believe something needs Lee personally, say so in the comment you leave with `needs-triage` and let triage make that call.
