...
namespace MaxWorlds.Tests.EditMode
{
    ...
    /// Tier 2) against World 2's own shipped a3 (two 3x30 m decks): (a) nothing built for a deck edge
    /// rises more than 0.10 m above the deck top; (b) every deck has post renderers at each corner and
    /// at no more than 4 m spacing along each edge; (c) nothing but those posts (and the deliberate
    /// flush ground-shadow decal) occupies the open volume between the floor and the slab underside
    /// inside the deck's own footprint; (d) the deck colliders are untouched.
    /// </summary>
    public sealed class MV821DeckWalkwayTests
    {
        ...
        [Test]
        public void WorldTwoArea3Decks_ReadAsOpenWalkwaysWithColliderUntouched()
        {
            ...
            List<DeckSlab> a3Decks = MapGeometry.Decks(map).Where(d => d.Id.StartsWith("a3_deck")).ToList();
            Assert.IsNotEmpty(a3Decks, "World 2's a3 must author at least one deck for this test to mean anything");
            ...
            try
            {
                ...
                foreach (DeckSlab deck in a3Decks)
                {
