...
namespace MaxWorlds.Tests.EditMode
{
    ...
    /// but nothing at all for <see cref="MaxWorlds.Arena.EntityKind.Replicator"/>, so World 2's 25
    /// Replicators (and the two hatches gated on all of them dying) had no way to be found on the map.
    ///
    /// Fails on base commit 1ac7f95 (before this ticket): <c>MapScreen</c> built zero GameObjects named
    /// "Replicator" for a World 2 map with 25 authored Replicator entities.
    ///
    ...
    /// Tier 2): (AC1) exactly 25 Replicator marker Images exist, each showing the alive fill colour;
    /// (AC2) killing one and reopening the map flips only that marker's resolved colour (fill alpha 0,
    /// outline #8C8C8C) while the other 24 stay unchanged; (AC3) the legend carries both new rows, by
    /// label text.
    /// </summary>
    public sealed class MV830ReplicatorMapMarkersTests
    {
        ...
        public void MapShowsEveryReplicator_AndFlipsOnlyTheDeadOneOnReopen()
        {
            ...
            Assert.AreEqual(25, build.Replicators.Count, "setup failure: World 2's shipped config must author 25 Replicators");
            ...
            // === AC1: exactly 25 Replicator markers, each showing the alive fill colour ===
            Image[] markers = ReplicatorMarkers(_screenGo);
            Assert.AreEqual(25, markers.Length, "MV-830 AC1: World 2 authors 25 Replicators — one map marker apiece");
            ...
            Assert.AreEqual(25, markers.Length, "MV-830 AC2: killing one must not remove or duplicate any marker");
            ...
            Assert.AreEqual(24, aliveCount, "MV-830 AC2: the other 24 markers must be unchanged");
