...
namespace MaxWorlds.Tests.EditMode
{
    ...
    public sealed class BossTests
    {
        ...
        /// escalates purely by seconds alive (1 + floor(aliveSeconds / 30), capped at 4 — MV-668 reverted
        /// MV-614's halving back to a 30s climb rate) — never by anything the player does — each level's
        /// brood volley composition draws only from that level's own set, and the brain's public surface
        /// no longer carries anything named "Charge" at all (not merely unreachable — gone).
        /// </summary>
        [TestCase(0f, 1)]
        [TestCase(29f, 1)]
        [TestCase(30f, 2)]
