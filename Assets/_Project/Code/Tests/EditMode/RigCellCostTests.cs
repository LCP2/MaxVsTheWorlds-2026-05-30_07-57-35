...
namespace MaxWorlds.Tests.EditMode
{
    ...
    /// <c>UpgradeCostCells</c> constant with a per-node-level escalation, while
    /// <see cref="CellSpend.UnlockCostCells"/> stays a flat 10 (down from 20) for every unlockable node
    /// — unlock breadth is already double-gated by part scarcity and the parent level &gt;= 2
    /// requirement, so it does not also escalate. MV-949: the escalation ladder shifted from
    /// 5/10/15/20 (capped at level 4) to 10/15/20/20 (capped at level 4, read off <c>level + 1</c>) —
    /// the old ladder's first step (5) undercut the 10-cell unlock cost that always precedes it, so a
    /// node's price visibly dropped right after unlock. Also pins the actual spend behavior at a
    /// mid-escalation level (AC3: a level-3 node's own upgrade), not just the pure formula —
    ...
    public sealed class RigCellCostTests
    {
        ...
        {
            Assert.That(CellSpend.UpgradeCostFor(1), Is.EqualTo(10));
            Assert.That(CellSpend.UpgradeCostFor(2), Is.EqualTo(15));
            Assert.That(CellSpend.UpgradeCostFor(3), Is.EqualTo(20));
            ...
            // level 3, where its own next upgrade must cost UpgradeCostFor(3) = 20.
            ...
            Assert.That(CellSpend.TryUpgradeNode("p_dmg"), Is.False, "one cell short of a level-3 node's own cost (20) must refuse");
            Assert.That(RigState.Level("p_dmg"), Is.EqualTo(3));
            Assert.That(PickupWallet.PowerCells, Is.EqualTo(19), "a rejected upgrade must not spend cells");
            ...
            Assert.That(PickupWallet.PowerCells, Is.EqualTo(0), "the upgrade must cost exactly UpgradeCostFor(3) = 20 cells");
        }
    }
}
