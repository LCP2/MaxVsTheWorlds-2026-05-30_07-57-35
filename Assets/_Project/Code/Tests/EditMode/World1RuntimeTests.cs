...
namespace MaxWorlds.Tests.EditMode
{
    ...
    public sealed class World1RuntimeTests
    {
        ...
        {
            // MV-650: Lee scaled Big Bermuda to 75% (6x6 -> 4.5x4.5) on the post-MV-613 pipeline.
            // Asserts the RESOLVED, constructed body — not the JSON — because a config edit that
            // never reached MapRuntime.BuildBoss would pass a JSON-only assertion while the scene
            // still spawned a mis-sized cube.
            Assert.IsTrue(WorldMapLoader.TryLoad(LoadWorld1(), out MapData map, out string reason), reason);
