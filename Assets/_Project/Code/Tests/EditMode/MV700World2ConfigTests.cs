    ...
    public sealed class MV700World2ConfigTests
    {
        ...
        public void World2Config_ResolvesFullDesign_ReplicatorGatesOverlaysAndBoss()
        {
            ...
            foreach (var (overlayId, targetId) in new[] { ("a19", "a3"), ("a15", "a6"), ("a13", "a11") })
            {
                ...
                MapZone overlayZone = map.Zone(overlayId);
                MapZone targetZone = map.Zone(targetId);
                Assert.IsNotNull(overlayZone, $"zone '{overlayId}' was never built");
                Assert.IsNotNull(targetZone, $"zone '{targetId}' was never built");
