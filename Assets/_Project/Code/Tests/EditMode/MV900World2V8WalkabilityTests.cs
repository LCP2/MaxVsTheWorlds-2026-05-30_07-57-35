...
namespace MaxWorlds.Tests.EditMode
{
    ...
    /// <c>decks</c> gets a SECOND unit for that deck surface — an overlay area's decks (a15/a17) ARE
    /// that area's whole unit, an in-place deck (a10-a14, a16, a18, a21) is layered on the same zone
    /// as its own floor. A unit's openings are every gate mouth built at that level (floor gates for a
    /// floor unit, "[DECK]" gates for a deck unit — <see cref="MapEntity.level"/>) plus every ramp
    /// endpoint authored in that area (<see cref="MapGeometry.Ramps"/>'s bottom -&gt; the floor unit,
    /// top -&gt; the deck unit). The walkable surface is rasterised on a 0.25 m grid and probed with
    ...
    /// the same connected component as every other.
    ///
    ...
    /// V8's config removes that cover block (Lee's 2026-09-24 sign-off).
    /// </summary>
    public sealed class MV900World2V8WalkabilityTests
    {
        ...
        private static Dictionary<string, Unit> BuildUnits(WorldConfig cfg, MapData map)
        {
            ...
            foreach (WorldArea a in cfg.areas)
            {
                ...
                foreach (WorldDeck d in a.decks)
                {
                    ...
                    u.Patches.Add((rect, slab.TopY));
                }
                if (u.Patches.Count > 0) units[u.Key] = u;
            }

            // Gate openings — floor gates feed a floor unit, "[DECK]" gates (MapEntity.level > 0) feed
            // the deck unit of the SAME owner area on each side of the link.
            foreach (MapLink link in map.links)
            {
                ...
                string levelTag = gate.level > 0 ? "deck" : "floor";
                AddOpening(units, ownerOfZone, link.from, levelTag, mouth);
                AddOpening(units, ownerOfZone, link.to, levelTag, mouth);
            }

            // Ramp openings — the bottom feeds the ramp's own area's floor unit, the top feeds that
            // same area's deck unit (WorldMapLoader only ever matches a ramp against decks authored in
            // its own area's `decks` array).
            foreach (RampSlab ramp in MapGeometry.Ramps(map))
            {
                ...
                if (units.TryGetValue($"{owner.id}:deck", out Unit deckUnit))
                    deckUnit.Openings.Add(new Vector2(ramp.TopCenter.x, ramp.TopCenter.z));
            }

            return units;
        }

        private static void AddOpening(Dictionary<string, Unit> units, Dictionary<string, string> ownerOfZone,
            string zoneId, string levelTag, Vector2 point)
        {
            if (!ownerOfZone.TryGetValue(zoneId, out string ownerId)) return;
            if (units.TryGetValue($"{ownerId}:{levelTag}", out Unit u)) u.Openings.Add(point);
        }
