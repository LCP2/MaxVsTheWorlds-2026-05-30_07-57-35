...
namespace MaxWorlds.Arena
{
    ...
    public sealed class MapStaticBatchRoot : MonoBehaviour
    {
        ...
        private void TagDressingSludge()
        {
            ...
            // method's own call site (above) for why the narrower Sludge-only walk left every other
            // dressing renderer permanently enabled regardless of the area gate.
    ...
    public static class MapRuntime
    {
        ...
        /// footprint) — those fall back to the nearest floor zone by centre distance rather than leaking
        /// through the area gate as permanently-enabled, the same fallback <see cref="MapStaticBatchRoot"/>'s
        /// own dressing tagging uses for the same reason.</summary>
        ...
        /// <summary>MV-904: the nearest zone to (<paramref name="px"/>, <paramref name="pz"/>) by
        /// distance to its own footprint rect (0 if the point already falls inside it, which never
        /// happens for a caller of this method — see below) — the shared fallback both
        /// <see cref="TagStatic"/> and <see cref="MapStaticBatchRoot"/>'s own dressing tagging use for a
        /// renderer <see cref="MapData.ZoneAt(float,float,float)"/>'s exact containment test resolves to
        /// no zone at all (a kerb/pipe/overhead run hugging a long elevated corridor's OWN walls, where
        /// no floor zone sits underneath it — measured on World 2's own "area16", a 106 m deck with no
        /// underlying room), so it gates with whichever zone it visually reads as belonging to instead of
        /// leaking through the area gate as permanently-enabled forever.
        ///
        ...
        /// gate's own measured effect (see this ticket's Jira comment for the count that caught it).
        /// Rect distance rather than centre distance so a long or irregularly-shaped zone (the same 106 m
