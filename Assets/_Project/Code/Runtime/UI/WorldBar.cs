...
namespace MaxWorlds.UI
{
    ...
    public static class WorldBar
    {
        ...
        /// again no matter what the body it hangs on is scaled to. The same trap later bit the
        /// robots (YT-74), so the maths lives in <see cref="ParentScale"/> now — one concept, one
        /// place.</summary>
        public static Vector3 Unscale(Vector3 parentLossyScale) => ParentScale.Unscale(parentLossyScale);
        ...
        public static float LocalOffsetY(float worldMetres, float parentScaleY)
            => ParentScale.LocalOffset(worldMetres, parentScaleY);
