...
{
    /// <summary>
    /// Drives the Max detail-pass capture (MV-453) — one 1920x1080 screenshot per compass yaw via
    /// <c>MaxWorlds.Dev.MaxDetailDirector</c>. Same harness shape as <see cref="RobotRosterCapture"/>,
    /// including its MV-444 fix (a headless-survives-domain-reload marker), and its own distinct
    /// arm/marker files so it never collides with the other capture directors on the same run.
    ///
    ///   Unity.exe -batchmode -projectPath . -executeMethod MaxWorlds.Editor.MaxDetailCapture.CaptureAll
    ///
    /// (NO -nographics: the capture needs a GL context; NO -quit: it exits itself once the shots exist.)
    /// </summary>
    public static class MaxDetailCapture
    {
        private const string Scene = "Assets/_Project/Scenes/Backyard_Slice.unity";
        private const string ArmFile = "Temp/maxdetail.arm";
        private const string HeadlessMarker = "Temp/maxdetail.headless";
        private const double TimeoutSeconds = 120;

        private static string OutDir =>
            Path.GetFullPath(Path.Combine(Application.dataPath, "..", "docs", "press", "max-detail"));
        private static string DoneFile => Path.Combine(OutDir, "_maxdetail_done.txt");

        [MenuItem("MaxWorlds/Capture Max Detail")]
        public static void CaptureFromMenu()
        {
            Arm();
            OpenScene();
            EditorApplication.EnterPlaymode();
            Debug.Log("[MaxDetail] filming — the editor will write PNGs to docs/press/max-detail/ and stop play mode itself.");
        ...
        public static void CaptureAll()
        {
            ...
            {
                Debug.LogError("[MaxDetail] CaptureAll failed to start: " + e);
        ...
        private static void PollHeadless()
        {
            if (File.Exists(DoneFile))
            {
                ...
                EditorApplication.update -= PollHeadless;
                Disarm();
                Debug.Log($"[MaxDetail] done ({(ok ? "ok" : "fail")}). Marker:\n{status}");
            ...
            if (EditorApplication.timeSinceStartup > _deadline)
            {
                EditorApplication.update -= PollHeadless;
                Disarm();
                Debug.LogError("[MaxDetail] timed out waiting for capture to finish.");
                if (EditorApplication.isPlaying) EditorApplication.isPlaying = false;
                EditorApplication.Exit(1);
            }
        }
        ...
        private static void PollMenu()
        {
            ...
            if (done || EditorApplication.timeSinceStartup > _deadline)
            {
                ...
                if (EditorApplication.isPlaying) EditorApplication.isPlaying = false;
                Debug.Log(done
                    ? "[MaxDetail] done — see docs/press/max-detail/. Marker:\n" + SafeRead(DoneFile)
                    : "[MaxDetail] timed out.");
            }
        }
